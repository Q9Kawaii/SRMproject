...
export default function StudentsDashBoard({ regNo: propRegNo, section: propSection }) {
  ...
  const generatePDF = () => {
    ...
    doc.text(`Basic Profile Score: ${calculateTotalScore()}/5.0`, 14, finalY);
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      // 1. Calculate basic score (only for 10th and 12th)
                    ...
                    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 backdrop-blur-sm rounded-2xl p-6 border-2 border-blue-200">
                      <div className="flex items-center gap-3 mb-6">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">Add your certifications, awards, and achievements</p>
                        </div>
                      </div>
                      ...
                      {Object.entries(studentData.achievementsMap || {}).map(([key, value]) => {
                        ...
                        return (
                          <div key={key} className="mb-6 p-4 bg-white/80 rounded-xl border border-blue-100">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              <input
                                type="url"
                                placeholder="Proof/Certificate Link (Google Drive/OneDrive)"
                                value={link}
                                onChange={(e) => {
                                  const updatedMap = { ...studentData.achievementsMap };
                      ...
                      <button
                        type="button"
                        className="mt-4 px-6 py-3 bg-gradient-to-r from-[#0c4da2] to-[#3a5b72] text-white font-bold rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-300 flex items-center"
                        onClick={() => {
                          const newKey = `achi${Date.now()}`;
                          const updatedMap = {
                            ...(studentData.achievementsMap || {}),
                            [newKey]: `~~0~${new Date().toISOString()}`,
                          };
                          setStudentData((prev) => ({ ...prev, achievementsMap: updatedMap }));
                        }}
                      >
